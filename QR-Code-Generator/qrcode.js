...
(function () {
	// -------------------------------------------------------------
	// بخش اصلی کتابخانه (بدون تغییر در منطق QR)
	// -------------------------------------------------------------
	...
	// SVG drawer (در اندروید استفاده نمی‌شود، اما برای حفظ ساختار)
	var svgDrawer = (function() {
		...
		Drawing.prototype.draw = function (oQRCode) {
			...
			svg.appendChild(makeSVG("circle", {"fill": _htOption.colorDark, "cx": "0.5", "cy": "0.5", "r": "0.4", "id": "template"}));
	...
	// =============================================================
	// نسخه Canvas با کیفیت 720 (افزایش رزولیشن پیش‌فرض به 720x720)
	// =============================================================
	var Drawing = !_isSupportCanvas() ? (function () {
		// fallback برای مرورگرهای خیلی قدیمی (جدول) - در اندروید جدید استفاده نمی‌شود
		var Drawing = function (el, htOption) {
			this._el = el;
			this._htOption = htOption;
		};
		...
		// تابع رسم مستطیل با گوشه‌های گرد (برای الگوهای موقعیت)
		...
		// تابع رسم الگوی موقعیت یکپارچه با سه لایه و گوشه‌های گرد
		function drawFinderPattern(ctx, x, y, moduleSize, colorDark, colorLight, cornerRadius) {
			var size = 7 * moduleSize;
			var midSize = 5 * moduleSize;
			...
			// لایه میانی روشن
			ctx.fillStyle = colorLight;
			roundedRect(ctx, x + moduleSize, y + moduleSize, midSize, midSize, radius * 0.8);
			ctx.fill();
			
			// مرکز تیره
			ctx.fillStyle = colorDark;
			roundedRect(ctx, x + 2 * moduleSize, y + 2 * moduleSize, innerSize, innerSize, radius * 0.6);
			ctx.fill();
		}
		...
		Drawing.prototype.draw = function (oQRCode) {
			...
			var nCount = oQRCode.getModuleCount();
			var moduleWidth = _htOption.width / nCount;
			var moduleHeight = _htOption.height / nCount;
			var dotRadius = Math.min(moduleWidth, moduleHeight) * 0.35;
			var cornerRadius = moduleWidth * 0.4;
			
			var probeSize = 7;
			var probes = [
				{ row: 0, col: 0 },
				{ row: 0, col: nCount - probeSize },
				{ row: nCount - probeSize, col: 0 }
			];
			
			function isInProbe(row, col) {
				for (var p = 0; p < probes.length; p++) {
					var r0 = probes[p].row;
					var c0 = probes[p].col;
					if (row >= r0 && row < r0 + probeSize && col >= c0 && col < c0 + probeSize) {
						return true;
					}
				}
				return false;
			...
			this.clear();
			// پس‌زمینه روشن کل بوم
			_oContext.fillStyle = _htOption.colorLight;
			_oContext.fillRect(0, 0, _htOption.width, _htOption.height);
			
			// رسم دایره‌های ماژول‌های معمولی (به جز نواحی الگوها)
			for (var row = 0; row < nCount; row++) {
				for (var col = 0; col < nCount; col++) {
					if (!oQRCode.isDark(row, col)) continue;
					if (isInProbe(row, col)) continue;
					
					var x = col * moduleWidth;
					var y = row * moduleHeight;
					var centerX = x + moduleWidth / 2;
					var centerY = y + moduleHeight / 2;
					
					_oContext.beginPath();
					_oContext.arc(centerX, centerY, dotRadius, 0, Math.PI * 2);
					_oContext.fillStyle = _htOption.colorDark;
					_oContext.fill();
				}
			}
			
			// رسم الگوهای موقعیت به صورت یکپارچه
			for (var p = 0; p < probes.length; p++) {
				var startRow = probes[p].row;
				var startCol = probes[p].col;
				var x = startCol * moduleWidth;
				var y = startRow * moduleHeight;
				drawFinderPattern(_oContext, x, y, moduleWidth, _htOption.colorDark, _htOption.colorLight, cornerRadius);
			}
		...
		Drawing.prototype.makeImage = function () {
			if (this._bIsPainted && (!this._android || this._android >= 3)) {
	...
	QRCode = function (el, vOption) {
		this._htOption = {
			width : 7680,          // تغییر: اندازه پیش‌فرض 720 برای کیفیت بالا
			height : 7680,         // تغییر: اندازه پیش‌فرض 720
			typeNumber : 4,
			colorDark : "#000000",
			colorLight : "#ffffff",
